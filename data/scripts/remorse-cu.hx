...
function create() {
	...
	// }
	// if (mania == 4 || mania == 0 || mania == null || mania != 6) {
	// 	disableScript();
	// }
	mania = 6;
	// trace(PlayState.instance.difficulty);
	// if(PlayState.instance.difficulty.toLowerCase() == "6k"){
	// 	disableScript();
	// }
}

function onNoteCreation(e) {
	if(strumsToDisable.contains(e.strumLineID)){
		// trace(e.strumLineID, "Neutralized");
		return;
...
function onStrumCreation(e) {
	if(e.strum.strumLine.characters == ["disablethis"]){
		e.strum.strumLine.characters[0].visible = false;
		return;
	}
	e.cancel();
	var strum = e.strum;
